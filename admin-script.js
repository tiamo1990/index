// Toast 提示函数（admin 专用）
function adminToast(message, duration = 3000) {
    let toast = document.getElementById('admin-toast-message');
    if (!toast) {
        toast = document.createElement('div');
        toast.id = 'admin-toast-message';
        toast.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:rgba(15,23,42,0.95);color:#fff;padding:12px 28px;border-radius:12px;font-size:0.95rem;z-index:99999;backdrop-filter:blur(12px);box-shadow:0 8px 32px rgba(0,0,0,0.4);opacity:0;transition:opacity 0.3s ease;pointer-events:none;white-space:pre-line;max-width:90vw;';
...
const USER_GITHUB = {
    ...
    repo: 'index',           // 仓库名
    token: 'YOUR_GITHUB_TOKEN'  // GitHub Personal Access Token — 请替换为环境变量或服务端注入
};
...
function showGitHubSetupModal() {
    ...
    const defaultConfig = { 
        owner: 'tiamo1990', 
        repo: 'index', 
        token: 'YOUR_GITHUB_TOKEN'  // 请替换为环境变量或服务端注入 
    };
